...
func GenerateBrief(ctx context.Context, cfg LLMConfig, items []feeds.NewsItem) (*Brief, error) {
	if cfg.APIKey == "" && cfg.Provider != ProviderLocal {
		return &Brief{
...
func parseLocalBriefResponse(content string) string {
	...
	for _, line := range lines {
		...
		clean := strings.ReplaceAll(upper, "*", "")
		clean = strings.ReplaceAll(clean, "#", "")
		clean = strings.TrimSpace(clean)

		if !inSummary && strings.HasPrefix(clean, "SUMMARY:") {
			inSummary = true
			idx := strings.Index(upper, "SUMMARY:")
			after := strings.TrimSpace(trimmed[idx+8:])
			if after != "" {
				summaryLines = append(summaryLines, after)
			}
			continue
		}
		if inSummary && trimmed != "" {
			if strings.HasPrefix(clean, "THREATS:") || strings.HasPrefix(clean, "COUNTRY") {
				break
			}
...
func parseBriefResponse(content string) (string, []string, []CountryRisk) {
	...
	for _, line := range strings.Split(content, "\n") {
		...
		if strings.HasPrefix(clean, "SUMMARY:") {
			if current != "" {
			...
			idx := strings.Index(upper, "SUMMARY:")
			after := strings.TrimSpace(trimmed[idx+8:])
			if after != "" { buf.WriteString(after + "\n") }
		} else if strings.HasPrefix(clean, "THREATS:") {
			if current != "" {
				sections[current] = strings.TrimSpace(buf.String())
			}
			current = "THREATS"
			buf.Reset()
		} else if strings.HasPrefix(clean, "COUNTRY_RISKS:") || strings.HasPrefix(clean, "COUNTRYRISKS:") || strings.HasPrefix(clean, "COUNTRY RISKS:") {
			if current != "" {
				sections[current] = strings.TrimSpace(buf.String())
			}
